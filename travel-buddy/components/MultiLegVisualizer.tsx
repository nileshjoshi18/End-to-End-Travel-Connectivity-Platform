...
interface MultiLegInfo {
  start_stop: string
  end_stop: string
  source_station: string   // now returned directly by API
  dest_station: string     // now returned directly by API
  requested_time: string
  final_arrival: string
  legs: Leg[]
  leg_fares: number[]      // train fare per leg, from API
  source_distance: string  // injected from connectivity response
  source_walk: string
...
export default function MultiLegVisualizer({ info, src, dest }: Props) {
  ...
  // Auto fares (walk legs) — client-calculated from distance
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="flex items-center gap-2 mb-5">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400">
              {info.legs.length} train{info.legs.length !== 1 ? "s" : ""} · direct route unavailable
            </p>
          </div>
        ...
        <div className="flex items-stretch gap-0 overflow-x-auto pb-1">
          ...
          <div className="flex flex-col items-center flex-shrink-0 min-w-[64px]">
            ...
            {autoFareSrc > 0 && <p className="text-[9px] font-bold text-blue-600">₹{autoFareSrc}</p>}
          </div>

          {info.legs.map((leg, i) => {
            const style  = getStyle(leg.line)
            const isLast = i === info.legs.length - 1
            const fare   = legFares[i]
            return (
          ...
          <div className="flex flex-col items-center flex-shrink-0 min-w-[64px]">
            ...
            {autoFareDest > 0 && <p className="text-[9px] font-bold text-blue-600">₹{autoFareDest}</p>}
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1">
            ...
            {legFares.map((f, i) => (
              <span key={i} className="text-[11px] text-gray-400">
                🚂 Leg {i + 1} ({info.legs[i]?.line}) <span className="font-bold text-gray-600">₹{f}</span>
              </span>
            ))}
            {autoFareDest > 0 && (
              <span className="text-[11px] text-gray-400">
      ...
      <div className="space-y-3">
        {info.legs.map((leg, i) => {
          ...
          const fromIdx  = stops.findIndex((s: any) => s.stop_id === leg.from_stop || s.stop_name === leg.from_stop)
          const toIdx    = stops.findIndex((s: any) => s.stop_id === leg.to_stop   || s.stop_name === leg.to_stop)
            ...
            <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              >
                ...
                <div className="flex items-center gap-2 ml-3">
                  {fare != null && (
                    <span className="text-xs font-black bg-white/20 px-2 py-1 rounded-lg tabular-nums">₹{fare}</span>
                  )}
                ...
                <div className="px-5 py-3">
                  <div className="relative">
                    ...
                    {legStops.map((stop, si) => {
                      ...
                      return (
                        <div key={stop.stop_id ?? si} className="relative flex items-center gap-3 py-1.5">
