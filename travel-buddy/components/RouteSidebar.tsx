...
import { getLineStyle, isMetroStop } from "@/utils/transitDesign"
...
export default function RouteSidebar({
  ...
  const crowdFor = (stopId: string | undefined, scores: Record<string, { crowd_score: number | null }> | undefined) => {
    if (!stopId || !scores) return null
    return scores[stopId]?.crowd_score ?? null
  }

  return (
    <div className="w-80 h-full bg-white shadow-xl flex flex-col overflow-hidden border-r border-gray-100">

      {/* Header — departure-board styling */}
      <div className="bg-[#0b1220] px-6 py-5 relative overflow-hidden">
        <div className="absolute inset-0 opacity-[0.04] bg-[repeating-linear-gradient(90deg,white_0px,white_1px,transparent_1px,transparent_8px)]" />
        <div className="relative flex items-center gap-2">
          <span className="w-6 h-6 rounded-full bg-amber-400 text-[#0b1220] text-[10px] font-black flex items-center justify-center">TB</span>
          <h2 className="text-base font-bold text-white tracking-wide">Wayfr</h2>
        </div>
        <p className="text-slate-400 text-[11px] mt-1.5 relative">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="p-3 bg-blue-50 rounded-xl border border-blue-100">
              ...
              <div className="mt-1.5">
                <CrowdBadge score={crowdFor((travelInfo as any).start_stop, travelInfo.crowd_scores)} compact />
              </div>
            </div>
            <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100">
              ...
              <div className="mt-1.5">
                <CrowdBadge score={crowdFor((travelInfo as any).end_stop, travelInfo.crowd_scores)} compact />
              </div>
            </div>
          </div>
                    ...
                    >
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          {style.isMetro && <MetroBadge line={train.line ?? ""} />}
                          <p className={`text-[9px] font-bold uppercase tracking-wide ${i === 0 ? "text-blue-200" : "text-gray-400"}`}>{train.train_id}</p>
                        </div>
                        <p className={`text-xl font-black tabular-nums font-mono ${i === 0 ? "text-white" : "text-gray-900"}`}>{train.departure}</p>
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        <p className={`text-xl font-black tabular-nums font-mono ${i === 0 ? "text-white" : "text-gray-900"}`}>{train.arrival}</p>
                      </div>
                    </div>
                  )
                })}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 bg-gray-50/30">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="p-3 bg-blue-50 rounded-xl border border-blue-100">
              ...
              <div className="mt-1.5">
                <CrowdBadge score={crowdFor(multiLegInfo.start_stop, multiLegInfo.crowd_scores)} compact />
              </div>
            </div>
            <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100">
              ...
              <div className="mt-1.5">
                <CrowdBadge score={crowdFor(multiLegInfo.end_stop, multiLegInfo.crowd_scores)} compact />
              </div>
            </div>
          </div>
            ...
            <div className="relative">
              ...
              <div className="space-y-2">
                {multiLegInfo.legs.map((leg, i) => {
                  const colors = getLineStyle(leg.line)
                  const isLast = i === multiLegInfo.legs.length - 1
                  const interchangeCrowd = !isLast ? crowdFor(leg.to_stop, multiLegInfo.crowd_scores) : null
                  ...
                  return (
                    <div key={i} className="relative z-10">
                      ...
                      <div
                        ...
                        className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all hover:scale-[1.01] hover:shadow-sm active:scale-[0.99] ${colors.pill.split(" ")[0]} ${colors.border}`}
                      >
                        {/* Dot or metro badge */}
                        {colors.isMetro
                          ? <MetroBadge line={leg.line} size="md" />
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {interchangeCrowd !== null && (
                            <div className="mt-1.5">
                              <CrowdBadge score={interchangeCrowd} compact />
                            </div>
                          )}
                        </div>
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className={`text-sm font-black tabular-nums font-mono ${colors.text}`}>{leg.departure}</p>
                          <p className="text-[9px] text-gray-400 font-bold my-0.5">↓</p>
                          <p className={`text-sm font-black tabular-nums font-mono ${colors.text}`}>{leg.arrival}</p>
                        </div>
                      </div>
                    </div>
                  )
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-emerald-600 rounded-xl text-white">
            ...
            <p className="text-xl font-black tabular-nums font-mono">{multiLegInfo.final_arrival}</p>
          </div>
        </div>
      )}
    </div>
